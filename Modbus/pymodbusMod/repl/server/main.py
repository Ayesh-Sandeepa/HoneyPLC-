...
import json
import click
from pymodbusMod.compat import IS_PYTHON3, PYTHON_VERSION
from pymodbusMod.framer.socket_framer import ModbusSocketFramer
from pymodbusMod.server.reactive.main import (
    ReactiveServer, DEFAULT_FRAMER, DEFUALT_HANDLERS)
from pymodbusMod.server.reactive.default_config import DEFUALT_CONFIG
from pymodbusMod.repl.server.cli import run_repl
...
@click.option("--verbose", is_flag=True,
              help="Run with debug logs enabled for pymodbusMod")
@click.pass_context
def server(ctx, host, web_port, broadcast_support, repl, verbose):
    ...
    pymodbusMod_logger = logging.getLogger("pymodbusMod")
    logging.basicConfig(format=FORMAT)
    logger = logging.getLogger(__name__)
    if verbose:
        pymodbusMod_logger.setLevel(logging.DEBUG)
        logger.setLevel(logging.DEBUG)
    else:
        pymodbusMod_logger.setLevel(logging.ERROR)
...
def run(ctx, modbus_server, modbus_framer, modbus_port, modbus_unit_id,
    ...
    """
    if not IS_PYTHON3:
        click.secho("pymodbusMod Server REPL not supported on python2", fg="read")
        exit(1)
